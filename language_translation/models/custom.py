...
class Transformer(nn.Module):
    ...
    def decode(self, encoder_output, src_mask, tgt, tgt_mask):
        ...
        mask = torch.triu(torch.ones((1, tgt_mask.shape[-1], tgt_mask.shape[-1]), device=tgt_mask.device), diagonal=1).type(torch.int) == 0
    ...
    def greedy_decode(self, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
        ...
        while True:
            ...
            decoder_mask = torch.triu(torch.ones((1, decoder_input.size(1), decoder_input.size(1)), device=device), diagonal=1).type(torch.int) == 0
